...
import type { SavedCollection, SavedTab, StorageRoot } from '../../lib/storage/schema';
...
function makeTab(overrides: Partial<SavedTab> = {}): SavedTab {
  return { id: 't1', url: 'https://example.com', title: 'Example', faviconUrl: null, addedAt: 1000, ...overrides };
}

function makeCollection(overrides: Partial<SavedCollection> = {}): SavedCollection {
  return {
    id: 'c1', name: 'My Collection', groupId: 'g1', chromeGroupColor: null,
    tabs: [makeTab()], createdAt: 1000, updatedAt: 1000, ...overrides,
  };
}
...
function makeRoot(collection: SavedCollection): StorageRoot {
  return {
    ...defaultRoot(),
    groups: {
      g1: { id: 'g1', name: 'Work', color: 'blue', collectionIds: ['c1'], createdAt: 1000, updatedAt: 1000 },
    },
...
const noopCardHandlers = {
  ...
  onAddTab: vi.fn(), onRemoveTab: vi.fn(), onDuplicateTab: vi.fn(),
};
...
describe('CollectionCard — Add Tab Form', () => {
  it('shows "Add tab" toggle when card is expanded', async () => {
    render(<CollectionCard collection={makeCollection()} {...noopCardHandlers} />);
    await expandCard();
  ...
  it('shows URL input after clicking "Add tab"', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Tab URL' })).toBeInTheDocument();
  });

  it('calls onAddTab with URL on valid submission', async () => {
    const onAddTab = vi.fn();
    render(<CollectionCard collection={makeCollection()} {...noopCardHandlers} onAddTab={onAddTab} />);
    await expandCard();
    await userEvent.click(screen.getByRole('button', { name: 'Add tab' }));
    await userEvent.type(screen.getByRole('textbox', { name: 'Tab URL' }), 'https://newsite.com');
  ...
  it('shows inline error for invalid URL and does not call onAddTab', async () => {
    ...
    await expandCard();
    await userEvent.click(screen.getByRole('button', { name: 'Add tab' }));
    await userEvent.type(screen.getByRole('textbox', { name: 'Tab URL' }), 'not-a-url');
    await userEvent.click(screen.getByRole('button', { name: 'Add' }));
  ...
  it('rejects ftp:// URLs as invalid', async () => {
    ...
    await expandCard();
    await userEvent.click(screen.getByRole('button', { name: 'Add tab' }));
    await userEvent.type(screen.getByRole('textbox', { name: 'Tab URL' }), 'ftp://example.com');
    await userEvent.click(screen.getByRole('button', { name: 'Add' }));
    expect(screen.getByRole('alert')).toBeInTheDocument();
    expect(onAddTab).not.toHaveBeenCalled();
  });
  ...
  it('hides form and clears URL after successful add', async () => {
    ...
    await userEvent.type(screen.getByRole('textbox', { name: 'Tab URL' }), 'https://newsite.com');
    await userEvent.click(screen.getByRole('button', { name: 'Add' }));
    expect(screen.queryByRole('textbox', { name: 'Tab URL' })).not.toBeInTheDocument();
  });
...
describe('CollectionCard — Tab Remove', () => {
  it('shows ConfirmDialog when Remove tab is clicked', async () => {
    const collection = makeCollection({ tabs: [makeTab({ title: 'Alpha' })] });
    render(<CollectionCard collection={collection} {...noopCardHandlers} />);
    await expandCard();
    await userEvent.click(screen.getByRole('button', { name: 'Remove tab Alpha' }));
    expect(screen.getByRole('alertdialog')).toBeInTheDocument();
  });

  it('calls onRemoveTab when removal is confirmed', async () => {
    const onRemoveTab = vi.fn();
    const collection = makeCollection({ tabs: [makeTab({ id: 't1', title: 'Alpha' })] });
    render(<CollectionCard collection={collection} {...noopCardHandlers} onRemoveTab={onRemoveTab} />);
    await expandCard();
    await userEvent.click(screen.getByRole('button', { name: 'Remove tab Alpha' }));
...
describe('CollectionCard — Tab Duplicate', () => {
  it('calls onDuplicateTab when Duplicate is clicked', async () => {
    ...
    const collection = makeCollection({ tabs: [makeTab({ id: 't1', title: 'Alpha' })] });
    render(<CollectionCard collection={collection} {...noopCardHandlers} onDuplicateTab={onDuplicateTab} />);
    await expandCard();
    await userEvent.click(screen.getByRole('button', { name: 'Duplicate tab Alpha' }));
...
describe('Workspace — tab storage.patch mutations', () => {
  it('patches storage when Add Tab is submitted with valid URL', async () => {
    ...
    await storage.patch((d) => { d.groups = { ...root.groups }; d.collections = { ...root.collections }; });
    ...
    await userEvent.click(screen.getByRole('article'));
    await userEvent.click(screen.getByRole('button', { name: 'Add tab' }));
    await userEvent.type(screen.getByRole('textbox', { name: 'Tab URL' }), 'https://added.com');
    await userEvent.click(screen.getByRole('button', { name: 'Add' }));
  ...
  it('patches storage when tab is removed (confirm)', async () => {
    const collection = makeCollection({ tabs: [makeTab({ id: 't1', title: 'ToRemove' })] });
    const root = makeRoot(collection);
    await storage.patch((d) => { d.groups = { ...root.groups }; d.collections = { ...root.collections }; });
    const patchSpy = vi.spyOn(storage, 'patch');
    render(<Workspace root={root} loading={false} />);
    ...
    await userEvent.click(screen.getByRole('article'));
    await userEvent.click(screen.getByRole('button', { name: 'Remove tab ToRemove' }));
    await userEvent.click(screen.getByRole('button', { name: 'Remove' }));
  ...
  it('patches storage when tab is duplicated', async () => {
    const collection = makeCollection({ tabs: [makeTab({ id: 't1', title: 'ToDup' })] });
    const root = makeRoot(collection);
    await storage.patch((d) => { d.groups = { ...root.groups }; d.collections = { ...root.collections }; });
    const patchSpy = vi.spyOn(storage, 'patch');
    render(<Workspace root={root} loading={false} />);
    ...
    await userEvent.click(screen.getByRole('article'));
    await userEvent.click(screen.getByRole('button', { name: 'Duplicate tab ToDup' }));

    expect(patchSpy).toHaveBeenCalled();
  });

  it('duplicate creates a tab with a new ID (not same as source)', async () => {
    const collection = makeCollection({ tabs: [makeTab({ id: 'original-id', title: 'Clone' })] });
    const root = makeRoot(collection);
    await storage.patch((d) => { d.groups = { ...root.groups }; d.collections = { ...root.collections }; });
    render(<Workspace root={root} loading={false} />);

    await userEvent.click(screen.getByRole('article'));
    await userEvent.click(screen.getByRole('button', { name: 'Duplicate tab Clone' }));
