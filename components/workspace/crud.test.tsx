...
import type { SavedCollection, SavedGroup, SavedTab, StorageRoot } from '../../lib/storage/schema';
...
function makeGroup(overrides: Partial<SavedGroup> = {}): SavedGroup {
  return {
    id: 'g1', name: 'Work', color: 'blue', collectionIds: ['c1'],
    createdAt: 1000, updatedAt: 1000, ...overrides,
  };
}

function makeRoot(overrides: Partial<StorageRoot> = {}): StorageRoot {
  return { ...defaultRoot(), groups: { g1: makeGroup() }, collections: { c1: makeCollection() }, ...overrides };
}
...
describe('CollectionCard — inline rename', () => {
  it('shows input with current name on double-click of title', async () => {
    ...
    await userEvent.dblClick(screen.getByRole('heading', { level: 3, name: 'Alpha' }));

    expect(screen.getByRole('textbox', { name: 'Rename collection' })).toHaveValue('Alpha');
  });

  it('calls onRename with new name on Enter', async () => {
    const onRename = vi.fn();
    const collection = makeCollection({ name: 'Alpha' });
    render(<CollectionCard collection={collection} onRename={onRename} onDelete={vi.fn()} />);
...
describe('CollectionCard — delete', () => {
  ...
  it('calls onDelete when ConfirmDialog is confirmed', async () => {
    ...
    const collection = makeCollection({ name: 'Alpha' });
    render(<CollectionCard collection={collection} onRename={vi.fn()} onDelete={onDelete} />);
...
describe('GroupSection — mutations', () => {
  ...
  it('shows ConfirmDialog when delete group is clicked', async () => {
    ...
    render(<GroupSection group={group} collections={[]} {...noopGroupHandlers} />);
  ...
  it('calls onDeleteGroup when group delete is confirmed', async () => {
    ...
    const group = makeGroup({ name: 'Work' });
    render(<GroupSection group={group} collections={[]} {...noopGroupHandlers} onDeleteGroup={onDeleteGroup} />);
