...
vi.mock("../../composables/useImportExport", () => ({
  useImportExport: () => ({
    error: mockError,
    setError: (val: string | null) => { mockError = val; },
    success: mockSuccess,
    setSuccess: (val: string | null) => { mockSuccess = val; },
...
describe("Settings", () => {
  ...
  it("runs handleImportFile on file selection and opens ConfirmDialog when valid", async () => {
    ...
    expect(screen.getByText("This will replace all your current data. Continue?")).toBeInTheDocument();
  });
  ...
  it("sets storage and success message when import is confirmed", async () => {
    ...
    // Dialog should close
    expect(screen.queryByRole("heading", { name: /confirm import/i })).not.toBeInTheDocument();
  });
