...
function makeRoot(
  collections: StorageRoot['collections'] = {},
): StorageRoot {
...
describe('Popup', () => {
  it('renders the save session button', () => {
    render(<Popup />);
    expect(screen.getByRole('button', { name: /save current session/i })).toBeInTheDocument();
  });
  ...
  it('calls sendToBackground with SAVE_WINDOW on save button click', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /save current session/i }));

    await waitFor(() => expect(mockSendToBackground).toHaveBeenCalledOnce());
    expect(mockSendToBackground).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'SAVE_WINDOW',
        payload: expect.objectContaining({ collectionName: expect.any(String) }),
      }),
    );
  });
  ...
  it('shows success toast when SAVE_WINDOW succeeds', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/saved as/i)).toBeInTheDocument());
  });

  it('shows error message when SAVE_WINDOW returns error', async () => {
    mockSendToBackground.mockResolvedValueOnce({ ok: false, error: 'Background SW error' });
