...
function Banner({ banner }: { banner: BannerState }) {
    ...
    return (
        <div
            className="tl-pp-saved-banner tl-pp-saved-banner--error"
            role="alert"
        >
            <Icon name="x" size={14} />
        ...
        <div className="tl-popup" role="dialog" aria-label="TabLocal popup">
                ...
                {loading ? (
                    ...
                    <div role="list" aria-label="Recent collections">
                        {recentCollections.map(
                            ({ collection: c, groupColor }) => (
                                <RecentCollectionRow
                                    key={c.id}
                                    collection={c}
                                    groupColor={groupColor}
                                    onClick={() =>
                                        void handleAddToCollection(c.id)
                                    }
                                />
                            ),
                        )}
            ...
            <div className="tl-pp-new">
                <input
                    ...
                    onKeyDown={(e) =>
                        e.key === "Enter" && void handleNewCollSave()
                    }
