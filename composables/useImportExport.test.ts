...
describe('useImportExport', () => {
  ...
  describe('importData', () => {
    ...
    it('rejects and sets error when file content is not valid JSON', async () => {
      const file = new File(['invalid json text'], 'backup.json', { type: 'application/json' });

      const { result } = renderHook(() => useImportExport());

      await act(async () => {
        await expect(result.current.handleImportFile(file)).rejects.toThrow();
      });
    ...
    it('rejects and sets error when file is a JSON object but lacks TabLocal structure', async () => {
      ...
      const file = new File([JSON.stringify(unrelatedObj)], 'backup.json', { type: 'application/json' });
      ...
      expect(result.current.error).toBe('Invalid TabLocal import file structure');
    });
  });
});
