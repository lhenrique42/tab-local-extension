  ...
  // Ensure groupOrder is valid and sessions is always first
  {
    ...
    if (!migrated.groupOrder) {
      const others = allGroupIds
        ...
        .sort((a, b) => (migrated.groups[a]?.createdAt ?? 0) - (migrated.groups[b]?.createdAt ?? 0));
      migrated.groupOrder = [SESSIONS_GROUP_ID, ...others];
    } else {
      const filtered = migrated.groupOrder.filter(id => migrated.groups[id]);
      for (const id of allGroupIds) {
        if (!filtered.includes(id)) filtered.push(id);
      }
      migrated.groupOrder = [SESSIONS_GROUP_ID, ...filtered.filter(id => id !== SESSIONS_GROUP_ID)];
    }
  }
